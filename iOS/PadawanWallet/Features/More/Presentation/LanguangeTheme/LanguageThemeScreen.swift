...
struct LanguageThemeScreen: View {
    ...
    init() {
        _viewModel = StateObject(wrappedValue: LanguageThemeScreenViewModel(languageManager: LanguageManager.shared))
    }
    ...
    var body: some View {
        BackgroundView {
            ScrollView {
                VStack(alignment: .leading, spacing: 12.0) {
                    Text(languageManager.localizedString("select_language"))
                        .font(Fonts.subtitle)

                    buildSection(
                        title: languageManager.localizedString("app_level_language"),
                    ...
                    buildSection(
                        title: languageManager.localizedString("color_theme"),
        ...
        .navigationTitle(languageManager.localizedString("change_language"))
    ...
    private func buildSection<T: LanguageThemeItemProtocol>(
        ...
        VStack(alignment: .leading) {
            ...
            buildItems(type, itemSelected: itemSelected, disableItems: disableItems)
        }
        ...
        ForEach(Array(T.allCases)) { item in
            Button {
                ...
                HStack(spacing: 12) {
                    ...
                    Text(item is PadawanLanguage
                         ? languageManager.displayName(for: item as! PadawanLanguage)
                         : item.rawValue
                    )
            ...
            .disabled(disableItems.contains(item))
        }
    }
}
